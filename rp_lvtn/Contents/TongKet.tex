...
chỉnh sửa và theo dõi toàn bộ sự kiện và vé sự kiện trên hệ thống.
...
\subsubsection*{(6) Gợi ý sự kiện thông minh -- Mô hình ML Collaborative Filtering}

\indent \indent Nhóm đã hoàn thiện toàn bộ pipeline gợi ý cá nhân hóa, bao gồm: (a) thu thập
dữ liệu tương tác người dùng thông qua model \texttt{Interaction} (click, purchase),
(b) xử lý và aggregate dữ liệu trong MongoDB với tính toán điểm tương tác,
mã hóa đặc trưng về tuổi, giới tính, thể loại sự kiện, (c) xuất ra file CSV
và upload lên \textbf{HuggingFace Hub}, (d) huấn luyện mô hình Collaborative Filtering
ngoại tuyến, và (e) triển khai API inference để phục vụ gợi ý theo thời gian thực
cho từng người dùng. Bên cạnh đó, \textbf{Google Gemini 2.5 Flash} được tích hợp
làm trợ lý ảo tư vấn sự kiện, cung cấp context động từ danh sách các sự kiện
đang mở bán để trả lời các câu hỏi tự nhiên bằng tiếng Việt.

\subsubsection*{(7) Dashboard thống kê}
...
\subsubsection*{(8) Đánh giá và phản hồi}
...
\subsubsection*{(9) Bảo mật tổng thể}
...
\begin{enumerate}[leftmargin=*, label=\textbf{\arabic*.}]
    ...
    \item \textbf{Hỗ trợ cổng thanh toán còn hạn chế.}
    Hiện tại hệ thống chỉ tích hợp PayOS. Để mở rộng thị trường và 
    cung cấp lựa chọn thanh toán đa dạng cho người dùng, cần bổ sung 
    hỗ trợ cho các phương thức thanh toán phổ biến khác tại Việt Nam 
    như VNPay, MoMo và ZaloPay.
...
\begin{enumerate}[leftmargin=*, label=\textbf{\arabic*.}]
    ...
    \item \textbf{Mở rộng tích hợp cổng thanh toán.}
    ...
    (refund) tự động khi sự kiện bị hủy hoặc người dùng yêu cầu.

    \item \textbf{Phát triển hệ thống quản lý mã giảm giá.}
    Xây dựng hệ thống quản lý voucher nâng cao với các loại chiết khấu:
    theo phần trăm, theo số tiền cố định, hoặc miễn phí vận chuyển (nếu có).
    Mã giảm giá sẽ có điều kiện áp dụng chi tiết (tối thiểu hóa đơn, chỉ
    áp dụng cho sự kiện cụ thể, giới hạn số lượng sử dụng, ngày hiệu lực)
    và được tích hợp toàn diện vào luồng thanh toán, đồng thời cung cấp
    công cụ quản lý cho ban tổ chức.
    ...
    \item \textbf{Phát triển chức năng team-based organizer.}
    Nâng cấp mô hình organizer để hỗ trợ nhiều thành viên trong một
    ban tổ chức, với hệ thống phân quyền chi tiết (quản lý sự kiện,
    xem báo cáo, quản lý thanh toán, v.v.), cho phép phân công công
    việc hiệu quả hơn trong các ban tổ chức quy mô lớn.

\end{enumerate}

\section*{Kết luận}

\indent \indent Nhìn lại toàn bộ quá trình thực hiện, nhóm
...
thanh toán điện tử, vé QR điện tử, email tự động, học máy collaborative filtering
và trí tuệ nhân tạo, được triển khai và vận hành thực tế trên nền tảng cloud.
Các hạn chế hiện tại chủ yếu là những tính năng nâng cao và nâng cấp kiến trúc
mà nhóm đã có kế hoạch rõ ràng nhưng chưa đủ thời gian hoàn thiện trong phạm vi
đồ án. Những hướng phát triển được đề xuất đều có cơ sở từ kiến trúc hệ thống
đã xây dựng, tạo nền tảng vững chắc để nhóm hoặc các thành viên tiếp theo có
thể phát triển liên tục sau khi kết thúc đồ án.