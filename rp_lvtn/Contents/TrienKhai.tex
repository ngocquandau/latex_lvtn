...
\end{tabularx}
\vspace{15pt}
\caption{Danh sách các công cụ và thư viện sử dụng trong Testing}
\end{table}
...
\small % Giảm kích thước chữ một chút để bảng gọn hơn
\begin{tabularx}{\textwidth}{@{} X l l >{\bfseries\color{green!60!black}}l @{}}
\toprule
\textbf{Test Case} & \textbf{Expected} & \textbf{Actual} & \textnormal{\textbf{Status}} \\ \midrule
...
verifyToken token hợp lệ & Gọi next() & Gọi next() & Passed \\ \addlinespace
verifyAdmin req.user chưa có & 401 & 401 & Passed \\
verifyAdmin user không admin & 403 & 403 & Passed \\
verifyAdmin user admin & Gọi next() & Gọi next() & Passed \\ \addlinespace
POST /api/user tạo user & 201, không mật khẩu & 201, không mật khẩu & Passed \\
POST /api/user/login đúng & Token & Token & Passed \\
POST /api/user/login sai & 401 & 401 & Passed \\ \addlinespace
GET /api/user/profile & Profile data & Profile data & Passed \\
GET /api/user user thường & 403 & 403 & Passed \\
GET /api/user admin & Danh sách user & Danh sách user & Passed \\ \addlinespace
POST /api/event thiếu token & 403 & 403 & Passed \\
POST /api/event user không admin & 403 & 403 & Passed \\
POST /api/event admin & Tạo event & Tạo event & Passed \\ \addlinespace
GET /api/event & Danh sách event & Danh sách event & Passed \\
POST /api/purchases & Tạo purchase pending & Tạo purchase pending & Passed \\
GET /api/purchases/my-tickets & Chỉ paid purchases & Chỉ paid purchases & Passed \\ \addlinespace
POST /api/payment/create-url thiếu token & 403 & 403 & Passed \\
POST /api/payment/create-url purchase không tồn tại & 404 & 404 & Passed \\
POST /api/payment/payos-webhook & Cập nhật paid & Cập nhật paid & Passed \\ \bottomrule
\end{tabularx}
\vspace{15pt}
\caption{Kết quả thực thi Test Case cho hệ thống}
\end{table}
